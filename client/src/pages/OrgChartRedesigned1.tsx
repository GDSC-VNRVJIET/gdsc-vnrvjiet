...
const data2026: OrgChartData = {
  ...
  lead: { role: "Lead", name: "Sri Kruthi", img: KruthiImg },
   coLead: {  role: "Co Lead",name: "Sri Manikanta",img: ManikantaImg},
...
const data2025: OrgChartData = {
  ...
  coLead:{ role: "Co Lead", name: "Sahithi Kolla", img: sahithiimg},
  domainLeads: [
    {
      role: "WebDev Lead",
      domain:"WebDev",
...
const data2024: OrgChartData = {
  ...
  coLead:{role: "Co Lead",name: "Rishab",img: colead},
  domainLeads: [
    {
      role: "WebDev Lead",
      domain:"WebDev",
...
const OrgChart = () => {
  ...
  const filteredLeads =
  activeDomain === "All"
    ? dataToDisplay.domainLeads.filter((p) => p.domain) // excludes Co-Lead
    : dataToDisplay.domainLeads.filter(
        (p) => p.domain === activeDomain
      );

const coLead = dataToDisplay.domainLeads.find(
  (p) => p.role === "Co Lead"
);
        ...
        <>
          ...
          <div className="flex justify-center gap-10 mb-16 flex-wrap">
            <div className="w-[320px]">
               <GlassCard
                  profileImage={dataToDisplay.lead.img}
                  personRole={dataToDisplay.lead.role}
                  personName={dataToDisplay.lead.name}
                />
                </div>

            <div className="w-[320px]">
             <GlassCard
      profileImage={dataToDisplay.coLead.img}
      personRole={dataToDisplay.coLead.role}
      personName={dataToDisplay.coLead.name}
    />
               </div>
  ...
  <h1 className="text-5xl font-bold mb-6">Domain Leads</h1>

  {/* Domain Navbar BELOW TITLE */}
  ...
  className={`mt-5 w-full ${
    activeDomain === "All"
      ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-center place-items-center"
      : "flex flex-wrap justify-center gap-10"
       }`}
       >
  {activeDomain === "All"
    ? filteredLeads.flatMap((person, index) => {
        const cards = [
          {
            img: person.img,
            name: person.name,
        ...
        if (person.secondLead) {
          cards.push({
            img: person.secondLead.img,
            name: person.secondLead.name,
            role: person.role,
          });
        }

        if (coLead && coLead.domain === person.domain) {
          cards.push({
            img: coLead.img,
            name: coLead.name,
            role: coLead.role,
        ...
        return cards.map((card, idx) => (
          <div key={`${index}-${idx}`} className="w-[320px]">
            <GlassCard
    ...
    : filteredLeads.map((person, index) => (
        <React.Fragment key={index}>
          ...
          {person.secondLead && (
            <div className="w-[320px]">
              <GlassCard
                profileImage={person.secondLead.img}
                personRole={person.role}
                personName={person.secondLead.name}
              />
            </div>
          )}

          {/* Co-Lead */}
          {coLead && coLead.domain === person.domain && (
            <div className="w-[320px]">
              <GlassCard
                profileImage={coLead.img}
                personRole={coLead.role}
                personName={coLead.name}
              />
            </div>
          )}

          {/* Coordinators (only in domain view) */}
          {person.coordinators?.map((coord, idx) => (
            <div key={idx} className="w-[320px]">
              <GlassCard
                profileImage={coord.img}
