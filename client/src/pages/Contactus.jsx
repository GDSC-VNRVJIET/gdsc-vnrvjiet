        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-10 mt-5'>
          ...
          <div className='p-5 mt-5 bg-green-600 rounded'>
            <form onSubmit={handleSubmit(sendQuery)}>
              <div className='flex space-x-3'>
                <div className="form-group mt-4 mb-1">
                  <input type="text" className="form-control w-full outline-none border-2 border-gray-200 bg-green-600 p-2 text-black font-medium text-sm placeholder-light-black rounded-full focus:bg-gray-200 focus:text-black" id="firstName" placeholder="First Name" {...register('firstName', { required: true })} />
                  {errors.firstName?.type === 'required' && <p className='text-red-600'>First Name is required</p>}
                </div>
                <div className="form-group mt-4 mb-1">
                  <input type="text" className="form-control w-full outline-none border-2 border-gray-200 bg-green-600 p-2 text-black font-medium text-sm placeholder-light-black rounded-full focus:bg-gray-200 focus:text-black" id="lastName" placeholder="Last Name" {...register('lastName', { required: true })} />
                  {errors.lastName?.type === 'required' && <p className='text-red-600'>Last Name is required</p>}
                </div>
              </div>
              <div className="form-group mt-4 mb-1">
                <input type="email" className="form-control w-full outline-none border-2 border-gray-200 bg-green-600 p-2 text-black font-medium text-sm placeholder-light-black rounded-full focus:bg-gray-200 focus:text-black" id="email" placeholder="Enter email" {...register('email', { required: true })} />
                {errors.email?.type === 'required' && <p className='text-red-600'>Email is required</p>}
              </div>
              <div className="form-group mt-4 mb-1">
                <textarea className="form-control w-full outline-none border-2 border-gray-200 bg-green-600 p-2 text-black font-medium text-sm placeholder-light-black  focus:bg-gray-200 focus:text-black" id="exampleFormControlTextarea1" rows="5" placeholder='Your Message' {...register('msg', { required: true, minLength: 5 })}></textarea>
              ...
              <button type="submit" className="btn w-full bg-white text-green-600 border-2 border-gray-200 p-2 rounded-full mt-4 hover:bg-none hover:text-black">Submit</button>
            </form>
          </div>
        </div>
      </div>
